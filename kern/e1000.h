#ifndef JOS_KERN_E1000_H
#define JOS_KERN_E1000_H

#endif	// JOS_KERN_E1000_H
#include <inc/types.h>
#include <inc/assert.h>
#include <inc/string.h>

#define E1000_STATUS   (0x00008/4)
#define E1000_TDBAL    (0x03800/4) /* TX Descriptor Base Address Low - RW */
#define E1000_TDBAH    (0x03804/4)  /* TX Descriptor Base Address High - RW */
#define E1000_TDLEN    (0x03808/4)  /* TX Descriptor Length - RW */
#define E1000_TDH      (0x03810/4)  /* TX Descriptor Head - RW */
#define E1000_TDT      (0x03818/4)  /* TX Descripotr Tail - RW */
#define E1000_TCTL     (0x00400/4)  /* TX Control - RW */
#define E1000_TIPG     (0x00410/4)  /* TX Inter-packet gap -RW */
#define E1000_RDBAL    (0x02800/4)  /* RX Descriptor Base Address Low - RW */
#define E1000_RDBAH    (0x02804/4)  /* RX Descriptor Base Address High - RW */
#define E1000_RDLEN    (0x02808/4)  /* RX Descriptor Length - RW */
#define E1000_RDH      (0x02810/4)  /* RX Descriptor Head - RW */
#define E1000_RDT      (0x02818/4)  /* RX Descriptor Tail - RW */
#define E1000_RA       (0x05400/4)  /* Receive Address - RW Array */
#define E1000_MTA      (0x05200/4)  /* Multicast Table Array - RW Array */
#define E1000_RCTL     (0x00100/4)  /* RX Control - RW */
#define E1000_TCTL_EN     0x00000002    /* enable tx */
#define E1000_TCTL_PSP    0x00000008    /* pad short packets */
#define E1000_TCTL_CT     0x00000ff0    /* collision threshold */
#define E1000_TCTL_COLD   0x003ff000    /* collision distance */
#define E1000_TXD_STAT_DD 0x00000001 /* Descriptor Done */
#define E1000_RCTL_EN     0x00000002    /* enable */
#define E1000_RCTL_BAM    0x00008000    /* broadcast enable */
#define E1000_RCTL_SZ_2048 0x00000000    /* rx buffer size 2048 */
#define E1000_RCTL_SECRC  0x04000000    /* Strip Ethernet CRC */
#define E1000_RAH_AV      0x80000000    /* Receive descriptor valid */
#define E1000_RCTL_LPE    0x00000020    /* long packet enable */
#define E1000_RCTL_RST    0x00000001    /* Software reset */
#define E1000_RCTL_LBM_NO 0x00000000    /* no loopback mode */
#define E1000_RCTL_SBP    0x00000004    /* store bad packet */
#define E1000_RXD_STAT_DD       0x01    /* Descriptor Done */
#define E1000_RXD_STAT_EOP      0x02    /* End of Packet */
#define E1000_TX_DESCS 64
#define E1000_RX_DESCS 128
#define E1000_TX_PKT_SIZE 2048
#define E1000_RX_PKT_SIZE 2048

//e1000 bar0 variable
volatile uint32_t *e1000_bar0_addr;
//send packet variable
char send_packet[E1000_TX_DESCS][E1000_TX_PKT_SIZE];
//recieve packet variable
char recieve_packet[E1000_RX_DESCS][E1000_RX_PKT_SIZE];

struct transmit_descriptor
{
	uint64_t buff_addr;
	uint16_t length;
	uint8_t cso;
	uint8_t cmd;
	uint8_t status;
	uint8_t css;
	uint16_t special;
};

struct recieve_descriptor
{
	uint64_t buff_addr;
	uint16_t length;
	uint16_t packet_checksum;
	uint8_t  status;
	uint8_t  errors;
	uint8_t  special;
};

void transmit_initalize(void);
int transmit_packet(char *packet,int len);
void recieve_initalize(void);
int recv_packet(char *data , int *len);